...
func getSandbox(context *cli.Context) (*sandbox.Sandbox, error) {
	...
	return sandbox.Load(root, id, debug)
}
...
func loadSandbox(ctx *cli.Context, id string, spec *specs.Spec) (*sandbox.Sandbox, error) {
	root := ctx.GlobalString("root")
	debug := ctx.GlobalBool("debug")
	if debug {
		log.Printf("enabeld debug param")
	}
	s, err := sandbox.Load(root, id, debug)
	if err != nil {
		...
		return sandbox.Create(root, id, config)
	}
	if !ctx.Bool("silent") {
		log.SetOutput(utils.MustOpenPipeFile(s.PipeName()))
	}
	return s, nil
}
...
func killSandbox(c *sandbox.Sandbox) error {
...
func sandboxSpec(spec *specs.Spec, cipher string) (*sandbox.Config, error) {
	cwd, err := os.Getwd()
	if err != nil {
		return nil, err
	}
	...
	if _, ok := spec.Annotations[sandbox.User]; !ok {
		spec.Annotations[sandbox.User] = "root"
	}
	if _, ok := spec.Annotations[sandbox.Cipher]; !ok {
		spec.Annotations[sandbox.Cipher] = cipher
	}
	labels := []string{}
	net := sandbox.Net{}
	...
	return &sandbox.Config{
		...
		Labels:               append(labels, "bundle="+cwd, "ip="+net.String(), sandbox.Addr+"="+net.IPAddress),
